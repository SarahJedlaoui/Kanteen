...
export default function Restaurant({ id }) {
    ...
    console.log('Effect before, id:', id);
    useEffect(() => {
        console.log('Effect running, id:', id);
        const fetchPersona = async () => {
            try {
                if (!id) {
                    throw new Error('ID is undefined');
                }
                const response = await axios.get(`https://leapback-d796b66e0016.herokuapp.com/api/persona/${id}`);
...
export async function getServerSideProps({ params }) {
  const [persona, setPersona] = useState<Persona | null>(null);
  // Fetch your data based on params.id
  const response = await fetch(`https://leapback-d796b66e0016.herokuapp.com/api/persona/${params.id}`, {
    method: 'GET',
    headers: {
